...
def cst_by_depth (ns:[](i64, node)) : [](i64, (i64, (i64, node))) =
    ...
    let depths = 
        loop (acc:(i64, (i64, node)), ns:[](i64, (i64, node))) = ((0, (0, #production (#Null))), []) for n in (zip (indices ns) ns) do
            let temp = track_depth acc n in 
            (temp, concat ns [temp])
    in
    --let isolate (dn:(i64, (i64, node))) : (i64, i64) = (dn.1.0, dn.0) in
    --let d = map isolate depths in
    let idepths = zip (indices depths.1) depths.1 in
    let find_parents (n:(i64, (i64, (i64, node)))) : (i64, (i64, (i64, node))) =
        --(index, (depth, (parent, node)))
        match n
        case (index, (depth, (_, nde))) ->
            let parent_n =
                loop (flag:bool, p:i64) = (true, index) while flag do 
                    match idepths[p]
                    case (0, _) -> (false, 0)
            ...
            in
            (index, (depth, (parent_n.1, nde))) 
    in
    let parented = map find_parents idepths in
    let get_key (n:(i64, (i64, (i64, node)))) = 
        match n
        case (_, (depth, (_, _))) -> depth
    in
    radix_sort_by_key get_key i64.num_bits i64.get_bit parented
